...
const ProductsGalleryPage: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [prodRes, catRes] = await Promise.all([
          axios.get(`${apiUrl}/gallery-products`),
          axios.get(`${apiUrl}/gallery-categories`)
        ]);
                    ...
                    <AnimatePresence>
                      {filteredAndSortedProducts.map((product) => (
                        <motion.div key={product.id} variants={itemVariants} layoutId={`product-${product.id}`}>
                          <ProductCard id={product.id} name={product.name} image={product.image} price={product.price} shopType="gallery" />
                        </motion.div>
                      ))}
                    </AnimatePresence>
                  </motion.div>
