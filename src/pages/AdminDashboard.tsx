...
import { LogOut, Plus, Edit, Trash2, X, Package, Tags, Star, ChevronRight, Layers } from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [wProd, wCat, gProd, gCat] = await Promise.all([
                        ...
                        <nav className="space-y-1 mb-6">
                            <button
                                ...
                                className={`w-full flex items-center px-4 py-3 rounded-xl transition-colors font-medium ${
                                    activeShop === 'wood' ? 'bg-gray-900 text-white shadow-md' : 'text-gray-600 hover:bg-gray-50'
                                }`}
                            >
                            ...
                            <button
                                ...
                                className={`w-full flex items-center px-4 py-3 rounded-xl transition-colors font-medium ${
                                    activeShop === 'gallery' ? 'bg-gray-900 text-white shadow-md' : 'text-gray-600 hover:bg-gray-50'
                                }`}
                            >
                                <Layers className="w-5 h-5 mr-3 text-current" /> Προϊόντα Γαλλερίας
                            </button>
                        </nav>
                ...
                <div className="flex-1">
                    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 min-h-[600px]">
                        ...
                        {activeTab === 'products' && (
                            <div>
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                                    <h2 className="text-2xl font-bold text-gray-900 leading-none mt-1">Προϊόντα - {activeShop === 'wood' ? 'Ξυλεία' : 'Γαλλερία'}</h2>
                        ...
                        {activeTab === 'categories' && (
                            <div>
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                                    <h2 className="text-2xl font-bold text-gray-900 leading-none mt-1">Δέντρο Κατηγοριών - {activeShop === 'wood' ? 'Ξυλεία' : 'Γαλλερία'}</h2>
